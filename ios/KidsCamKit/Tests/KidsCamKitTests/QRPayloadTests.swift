...
final class QRPayloadTests: XCTestCase {
    ...
    func testParsesTheVectorURL() throws {
        ...
        XCTAssertEqual(parsed.apiBaseURL.absoluteString, "https://api.kidscam.example")
        XCTAssertEqual(
    ...
    func testRoundTripsThroughEncodeAndParse() throws {
        let original = QRPayload(
            ...
            apiBaseURL: try XCTUnwrap(URL(string: "https://eu.api.kidscam.example"))
        )
    ...
    func testRejectsNonKidscamURLs() {
        assertParseError("https://example.com/pair?v=1", .notAPairingURL)
        assertParseError("kidscam://other?v=1", .notAPairingURL)
